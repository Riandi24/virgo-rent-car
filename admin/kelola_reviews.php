...
if (isset($_GET['hapus'])) {
    ...
    if ($photo_table && mysqli_num_rows($photo_table) > 0) {
        $foto_result = mysqli_query($koneksi, "SELECT foto_path FROM tbl_review_foto WHERE id_review = $id_hapus");
        if ($foto_result) {
            while ($foto = mysqli_fetch_assoc($foto_result)) {
                $path = '../uploads/' . $foto['foto_path'];
                if (isset($foto['foto_path']) && $foto['foto_path'] && file_exists($path)) {
                    unlink($path);
                }
            }
        }
        mysqli_query($koneksi, "DELETE FROM tbl_review_foto WHERE id_review = $id_hapus");
    }
...
<tr class="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors align-top">
                            ...
                            <td class="py-4 px-4" data-label="Foto">
                                <?php if (!empty($photos)): ?>
                                    <div class="flex flex-wrap gap-2">
                                        <?php foreach (array_slice($photos, 0, 3) as $foto): ?>
                                            <img src="../uploads/<?= htmlspecialchars($foto); ?>" alt="Foto review" class="admin-review-photo-trigger w-16 h-16 object-cover rounded-lg border border-slate-700/50 cursor-pointer hover:opacity-90 transition" data-image="../uploads/<?= htmlspecialchars($foto); ?>">
                                        <?php endforeach; ?>
                                    </div>
                                <?php else: ?>
    ...
    <div id="adminReviewImageModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 p-4">
        <div class="relative max-w-4xl w-full">
            <button type="button" id="closeAdminReviewImageModal" class="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-white text-slate-900 flex items-center justify-center shadow-lg hover:bg-slate-200">
                <i class="fas fa-times"></i>
            </button>
            <img id="adminReviewImageModalImg" src="" alt="Foto review lengkap" class="w-full max-h-[80vh] object-contain rounded-2xl border border-slate-700 bg-slate-900">
        </div>
    </div>

    <script>
        const adminReviewPhotoTriggers = document.querySelectorAll('.admin-review-photo-trigger');
        const adminReviewImageModal = document.getElementById('adminReviewImageModal');
        const adminReviewImageModalImg = document.getElementById('adminReviewImageModalImg');
        const closeAdminReviewImageModal = document.getElementById('closeAdminReviewImageModal');

        if (adminReviewPhotoTriggers.length && adminReviewImageModal && adminReviewImageModalImg && closeAdminReviewImageModal) {
            adminReviewPhotoTriggers.forEach(trigger => {
                trigger.addEventListener('click', () => {
                    adminReviewImageModalImg.src = trigger.dataset.image;
                    adminReviewImageModal.classList.remove('hidden');
                    adminReviewImageModal.classList.add('flex');
                });
            });

            closeAdminReviewImageModal.addEventListener('click', () => {
                adminReviewImageModal.classList.add('hidden');
                adminReviewImageModal.classList.remove('flex');
                adminReviewImageModalImg.src = '';
            });

            adminReviewImageModal.addEventListener('click', (event) => {
                if (event.target === adminReviewImageModal) {
                    adminReviewImageModal.classList.add('hidden');
                    adminReviewImageModal.classList.remove('flex');
                    adminReviewImageModalImg.src = '';
                }
